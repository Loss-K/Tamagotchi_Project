...
class testingthings():
    def __init__(self, StatName, Curvalue, PosValue,):
        self.Stat = StatName
        self.Prior = Curvalue #Original Value = 15
        self.After = PosValue #Whatever change = +10

    def startvalues(stat_title):
        db = sqlite3.connect(dbtest)
        cur = db.cursor()
        curvalue = 0

        get_latest_stat = "SELECT stat_after FROM stat_detail WHERE stat_name =? ORDER BY ID DESC LIMIT 1 "
        cur.execute(get_latest_stat, (stat_title,))
    ...
    def increase(svalue, valuechange, valuename):
        result = svalue + valuechange
        if result > 100:
            result = 100
        print(f"{valuename} went from {svalue} to {result}")
        return result

    def decrease(svalue, valuechange, valuename):
        result = svalue - valuechange
        print(f"{valuename} went from {svalue} to {result}")
        return result
...
def __init__():
    ...
    if not os.path.isfile(dbtest):
        ...
        Sleep = testingthings.startvalues("Sleep")
        Hunger = testingthings.startvalues("Hunger")
        Hydration = testingthings.startvalues("Hydrate")
...
#############################Hunger#########################
        if hungerchange > 0:
            prior_value = testingthings.startvalues("Hunger")
            #updated_value = doingmath("Hunger", prior_value, hungerchange, math)
            print(prior_value)

            match math:
                case True:
                    testingresult = testingthings.increase(prior_value, hungerchange, "Hunger")
                    print(testingresult)
                    degrade = False
                    update_table("Hunger", str(prior_value), str(testingresult), str(degrade), "False")
                    print("True Case Hunger Completed.")
                case False:
                    testingresult = testingthings.decrease(prior_value, hungerchange, "Hunger")
                    print(testingresult)
                    degrade = True
                    update_table("Hunger", str(prior_value), str(testingresult), str(degrade), "False")
